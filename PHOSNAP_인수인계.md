...
#### is_active 는 켜는 값이 아니라 계산되는 값이다 (FIX_35)

FIX_34 의 첫 판은 **승인되면 무조건 켰다.** 그건 틀렸다.
윤건식(지역 없음·상품 없음)이 그대로 고객 검색에 올라왔다.
들어가 봐야 예약할 게 하나도 없다.

승인은 **필요조건**이지 충분조건이 아니다.
`ensureArtistRecord` 가 애초에 `is_active: false` 로 만들면서
"필수 정보 입력 전까지 비노출" 이라고 적어둔 그대로다.
...
| 트리거 | 대상 | 언제 |
|---|---|---|
| `trg_guard_activation` | 공급자 4개 테이블 | 프로필 저장 때마다 자격대로 맞춘다 |
| `trg_activate_on_approval` | `user_roles` | 승인·반려·정지 |
| `trg_refresh_listing` | `packages` · `stylist_services` · `dress_items` · `venue_items` | 상품 추가·삭제 |

이러면 `활성 작가에 지역 없음`, `활성 작가에 상품 없음`,
...
`BEFORE UPDATE` 시점에는 **아직 커밋되지 않은 `new` 가 아니라 옛 행**이 보인다.
작가가 지역을 처음 입력하는 순간 옛 값(null)을 보고 계속 꺼둔다 —
저장은 되는데 영영 노출되지 않는다.

`guard_provider_activation` 은 그래서 `new` 의 값으로 직접 계산한다.
일괄 정리용 `refresh_provider_listing()` 은 커밋된 행을 보므로 함수를 써도 된다.

#### 공급자에게 이유를 말해준다

조건을 알려주지 않으면 "승인은 났는데 왜 예약이 안 들어오지" 가 된다.
그것도 실패를 숨기는 것이다.

`provider_listing_status(kind, id)` 가 무엇이 비었는지 돌려주고,
`<ListingStatus>` 컴포넌트가 작가·헤메 대시보드 상단에 띄운다.
...
`VERIFY.sql` 이 두 가지를 본다.

* `노출 자격과 실제가 불일치` — FAIL. 트리거가 도는 한 0이어야 한다.
* `승인됐지만 프로필 미완성` — WARN. 사고는 아니지만 연락해서
  채우게 해야 한다. 본인은 승인만 보고 기다리고 있다.
...
### 5-22. 딸려오는 것은 주인이 빠지면 같이 빠져야 한다 (2026-09-13)

#### 의상은 세 사람이 파는데, 파는 방식이 다르다

| 파는 사람 | 언제 의상 탭에 보이나 | 왜 |
|---|---|---|
| 의상 벤더 | **항상** | 옷만 빌려준다. 작가를 안 골라도 빌릴 수 있어야 한다 |
| 헤메 | 그 헤메를 담았을 때 | 본인이 현장에 들고 온다 |
| 작가 | 그 작가를 담았을 때 | 본인이 현장에 들고 온다 |

작가의 자체 헤메도 같다 — 그 작가를 담아야 헤메 탭에 붙는다.

#### 무엇이 샜나

`BookCompose` 는 주인을 담으면 옷을 목록에 **더해주기만** 했다.
빼는 쪽이 없었다.

```
사진작가1 담기  →  의상 탭에 "사진작가1 작가님 보유 한복" 생김  →  한복 담기
사진작가1 빼기  →  의상 탭에서는 한복이 사라짐
                 그런데 cart.dress 에는 그대로 남고 합계에도 8만원이 남는다
```

**아무도 촬영장에 안 가져오는 옷에 고객이 돈을 낸다.** 규칙 5-18 위반이다.
작가를 *다른 작가로 교체*할 때도, 헤메를 뺄 때도 같았다.

#### 고친 방법 — 호출부가 아니라 쓰기 지점 한 곳

담기·빼기·패키지 선택·재조회 네 군데를 각각 막으면
다섯 번째 경로가 생길 때 또 샌다. `setCart` 를 한 겹 감싸고
`normalizeCart()` 를 통과하게 했다.
...
벤더 의상은 두 칸이 다 `null` 이라 걸리지 않는다. 그대로 남는 게 맞다.

#### 조용히 지우는 것도 실패를 숨기는 것이다

옷이 말없이 사라지면 고객은 자기가 뭘 잘못 눌렀는지 모른다.
빠질 때 `dropNotice` 로 이유를 띄운다 —
"윤헤메 님이 가져오기로 한 의상이라 함께 빠졌습니다 — 웨딩드레스".

#### 남은 판단

의상 탭에서 **담기 전에는** 작가·헤메 보유 의상을 볼 수 없다.
서울 14시 조회면 벤더 옷 7벌만 보인다.
"사진작가1과 함께 예약 시" 라고 표시하며 미리 보여줄지는
아직 안 정했다. 지금은 안 보여주는 쪽.
...
#### 세 번째 — 사진이 있는데 '사진 없음' 으로 나왔다

FIX_42 를 넣고 다시 대조하다 찾았다.

```
앵커 헤메 33개 중 img 없음 33개 · 그런데 portfolio_images 있음 30개
```

`stylists` 테이블에는 **`img` 컬럼이 아예 없다.** 사진은
`portfolio_images` 에만 있다. 카드가 `img` 만 보고 있었으니
사진이 멀쩡히 있는 헤메 30명이 전부 회색 '사진 없음' 상자였다.

`coverOf()` 로 포트폴리오 첫 장을 대표사진으로 쓴다.
작가 포트폴리오는 모양이 다르므로(`[{ images, coverIdx }]`) 둘 다 받는다 —
`coverIdx` 는 드래그로 대표사진을 바꾸는 기능이 쓰는 칸이다.

고친 뒤: 헤메 사진없음 33 → 3 (윤헤메 메뉴 3개. 실제로 포트폴리오가 없다),
작가 33명 전원 대표사진 확보.
...
### 5-24. 고친 게 안 보일 때 무엇을 의심할 것인가 (2026-09-13)

#### 멀쩡한 코드를 의심하며 시간을 버렸다

헤메 대표사진을 고치고 푸시한 뒤 `/stylists` 를 봤더니 여전히
11장 전부 "사진 없음" 이었다. DB 대조로는 33 → 3 으로 줄었는데.

서비스워커를 해제하고 캐시를 비우고 다시 열었더니 정상이 됐다.
번들 이름도 `index-Bx2ZFCr9.js` → `index-Bvi0kZ9t.js` 로 바뀌었다.

**그런데 원인을 단정할 수 없었다.** 서비스워커 해제·캐시 삭제·
쿼리스트링 추가·추가 대기를 한꺼번에 해서 변수를 못 갈랐다.
그 사이에 Vercel 빌드가 끝났을 가능성도 똑같이 크다.

교훈은 "캐시가 범인이다" 가 아니라 **"구분할 방법이 없었다"** 는 것이다.

#### 화면이 자기 버전을 말하게 했다

```
브라우저 콘솔:  window.__BUILD__
→ { commit: '506b9cd', time: '2026-09-13T…' }
```

`vite.config.js` 의 `define` 이 빌드 시점에 심는다.
Vercel 은 `VERCEL_GIT_COMMIT_SHA` 를 주고, 로컬은 git 에게 묻는다.
둘 다 없으면 `'unknown'` — **거짓 값을 넣지 않는다.**

이제 푸시하고 고친 게 안 보이면 순서가 정해져 있다.

1. `window.__BUILD__.commit` 이 방금 푸시한 커밋인가?
   - 아니면 → 배포가 안 끝났거나 캐시다. 코드는 건드리지 마라.
   - 맞으면 → 코드 문제다. 그때부터 의심해라.

이 한 줄이 없으면 3번부터 의심하게 된다.

#### 서비스워커에 실재하던 결함

원인 규명과 별개로, `public/sw.js` 에 진짜 문제가 있었다.

```js
const CACHE_NAME = 'phosnap-v1';          // 영원히 v1

self.addEventListener('activate', ...      // 옛 캐시 정리 코드인데
  if (cacheName !== CACHE_NAME) delete     // 지울 대상이 자기 자신뿐이라
);                                         // 아무것도 안 지운다
```

그리고 `/` 와 `/index.html` 을 미리 캐시하고 있었다.

**HTML 을 캐시하면 언젠가 반드시 짝이 어긋난다.**
번들에는 해시가 붙어 내용이 바뀌면 이름이 바뀐다 — 캐시가 옛것을 줄 수
없다. 그런데 `index.html` 은 이름이 그대로다. 옛 HTML 이 캐시에 남으면
그것이 가리키는 옛 번들까지 통째로 되살아난다. 배포를 해도 앱이
과거로 돌아간다. 오류도 없고 화면도 멀쩡하다.

고친 내용:
- `CACHE_NAME` 을 `phosnap-v2` 로. **배포 내용을 바꿀 때마다 올린다.**
- `PRECACHE_URLS` 에서 `/` 와 `/index.html` 제거
- 네비게이션은 항상 네트워크. 실패할 때만 `offline.html`

오프라인이면 "오프라인입니다" 라고 정직하게 말하는 편이,
옛 앱을 멀쩡한 척 보여주는 것보다 낫다.
