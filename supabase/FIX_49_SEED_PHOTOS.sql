...
--     전통 한복 6      → 사막 사진
--
--   스크립트 주석은 "실제 작가 사진이 아님이 분명하다" 고 적어뒀지만,
--   화면에서는 가짜로 보이지 않고 **고장난 데이터로 보인다.** 예약 화면에서
--   제품 사진은 고객이 고르는 근거라 특히 그렇다.
--
-- 무엇을 하나
--   의상 33건 · 장소 13건의 사진을 분류에 맞는 Unsplash 사진으로 바꾼다.
...
-- 1. 분류별 사진 목록
--
--    한 분류에 여섯 장을 두고 행마다 돌려쓴다. 한 장만 쓰면 목록이
--    같은 사진으로 도배된다.
-- ───────────────────────────────────────────────────────────────────────
create temporary table _seed_photo (kind text, idx int, id text);

insert into _seed_photo (kind, idx, id) values
  -- 한복
...
-- ───────────────────────────────────────────────────────────────────────
-- 2. 의상 — image_url 과 images 를 함께 맞춘다
--
...
-- 3. 장소
--
--    venue_items.images 는 [{url}] 모양이다 (시드 스크립트 389행).
-- ───────────────────────────────────────────────────────────────────────
with target as (
  select v.id,
   ...
   where v.images::text like '%picsum%'
),
pick as (
  select t.id,
...
drop table _seed_photo;


-- ───────────────────────────────────────────────────────────────────────
-- 4. 확인 — 남은 picsum 이 0 이어야 한다
--
--  select category,
