-- ============================================================
-- PHOSNAP PATCH Part 1: 테이블 + 정책 + 인덱스
-- (함수/트리거/DO 블록 없음 — 안전하게 실행 가능)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.dress_vendors (
  ...
  name text NOT NULL, name_i18n jsonb DEFAULT '{}',
  bio text, bio_i18n jsonb DEFAULT '{}',
  location_id text, location_names jsonb DEFAULT '{}',
  categories text[] DEFAULT '{}', img text, contact_info jsonb DEFAULT '{}',
...
CREATE TABLE IF NOT EXISTS stylists (
  ...
  name_ko TEXT NOT NULL, name_en TEXT, name_ja TEXT, name_zh TEXT,
  specialty TEXT, description TEXT, phone TEXT, instagram TEXT,
  portfolio_images TEXT[] DEFAULT '{}', location_id TEXT,
...
CREATE TABLE IF NOT EXISTS stylist_services (
  ...
  name_ko TEXT NOT NULL, name_en TEXT, name_ja TEXT, name_zh TEXT,
  price INT NOT NULL DEFAULT 0, duration_minutes INT DEFAULT 60,
...
CREATE TABLE IF NOT EXISTS dress_items (
  ...
  name_ko TEXT NOT NULL, name_en TEXT, name_ja TEXT, name_zh TEXT,
  category TEXT NOT NULL DEFAULT 'hanbok', price INT NOT NULL DEFAULT 0,
  sizes TEXT[] DEFAULT '{}', color TEXT, image_url TEXT, images TEXT[] DEFAULT '{}',
  description TEXT, is_available BOOLEAN DEFAULT true, sort_order INT DEFAULT 0,
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.venue_items (
  ...
  capacity int DEFAULT 10, price int DEFAULT 0,
  price_unit text DEFAULT 'per_session' CHECK (price_unit IN ('per_hour','per_session','per_day')),
  images jsonb DEFAULT '[]', description text, description_i18n jsonb DEFAULT '{}',
  amenities text[] DEFAULT '{}', sort_order int DEFAULT 0, is_available boolean DEFAULT true,
  created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
);
...
ALTER TABLE bookings ADD CONSTRAINT bookings_status_check CHECK (status IN ('pending','confirmed','completed','cancelled','delivered','refunded'));
DROP POLICY IF EXISTS "내 예약 업데이트" ON public.bookings;
CREATE POLICY "내 예약 업데이트" ON public.bookings FOR UPDATE USING (auth.uid() = customer_id) WITH CHECK (auth.uid() = customer_id);
...
CREATE TABLE IF NOT EXISTS public.reviews (
  ...
  text text NOT NULL DEFAULT '', author_name text, lang text DEFAULT 'ko',
...
CREATE TABLE IF NOT EXISTS package_reviews (
  ...
  package_id UUID, photographer_id UUID NOT NULL,
...
CREATE TABLE IF NOT EXISTS photographer_reviews (
  ...
  customer_id UUID NOT NULL REFERENCES auth.users(id),
  rating INT NOT NULL CHECK (rating >= 1 AND rating <= 5),
  title TEXT, body TEXT, tags TEXT[] DEFAULT '{}', created_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS review_replies (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  review_id UUID NOT NULL, review_type TEXT NOT NULL CHECK (review_type IN ('package','photographer')),
  photographer_id UUID NOT NULL, body TEXT NOT NULL,
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS messages (
  ...
  content TEXT NOT NULL, read_at TIMESTAMPTZ, created_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.stylist_schedules (
  ...
  booking_id UUID REFERENCES public.bookings(id) ON DELETE SET NULL,
  date DATE NOT NULL, time_slot TEXT NOT NULL,
...
-- 13. 누락 테이블: user_roles
CREATE TABLE IF NOT EXISTS public.user_roles (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  role text NOT NULL, status text DEFAULT 'active' CHECK (status IN ('active','pending','suspended')),
  created_at timestamptz DEFAULT now(), UNIQUE(user_id, role)
);
...
-- 15. 누락 테이블: vendor_reviews
CREATE TABLE IF NOT EXISTS public.vendor_reviews (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  vendor_id uuid NOT NULL, vendor_type text NOT NULL CHECK (vendor_type IN ('dress','venue','stylist')),
  booking_id uuid REFERENCES public.bookings(id) ON DELETE SET NULL,
  customer_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  rating int NOT NULL CHECK (rating >= 1 AND rating <= 5),
  text text DEFAULT '', author_name text, lang text DEFAULT 'ko',
  is_visible boolean DEFAULT true,
  created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now()
);
...
-- 16. vendor_dresses 뷰
CREATE OR REPLACE VIEW public.vendor_dresses AS SELECT * FROM public.dress_items;

-- ✅ Part 1 완료! 다음으로 PATCH_PART2.sql을 실행하세요.
