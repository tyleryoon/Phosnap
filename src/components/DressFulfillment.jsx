...
export const FULFILLMENT_LABELS = {
  byOwner:  { ko: '주인이 현장에 들고 감', en: 'Brought by owner',   ja: '本人が現場へ持参', zh: '本人携带至现场' },
...
const labelStyle = {
  display: 'block', fontSize: '0.9rem', marginBottom: '0.5rem', color: 'var(--muted)',
};
const inputStyle = {
  width: '100%', padding: '0.75rem', backgroundColor: 'var(--bg)', color: 'var(--text)',
  border: '1px solid var(--gold-dim)', boxSizing: 'border-box', fontFamily: 'var(--font-sans)',
};

export default function DressFulfillment({ value, onChange, allowByOwner = false, lang = 'ko' }) {
  const methods = value?.fulfillment?.length
    ? value.fulfillment
    : (allowByOwner ? ['byOwner'] : ['pickup']);
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: '0.75rem', color: 'var(--muted)', marginBottom: '1.25rem', lineHeight: 1.7 }}>
        {allowByOwner
      ...
      {showDeposit && (
        <div style={{ marginBottom: '1.25rem' }}>
          ...
          <input
            ...
            value={deposit}
            onChange={e => onChange({ fulfillment: methods, deposit: num(e.target.value), deliveryFee })}
            placeholder="0"
            style={inputStyle}
          />
      ...
      {showDelivery && (
        <div style={{ marginBottom: '1.25rem' }}>
          ...
          <input
            ...
            value={deliveryFee}
            onChange={e => onChange({ fulfillment: methods, deposit, deliveryFee: num(e.target.value) })}
            placeholder="0"
            style={inputStyle}
          />
        </div>
