...
const getLocCountry = (loc) => {
  ...
  const ovr =
    OVERSEAS_REGIONS.find((r) => r.id === loc.regionId) ||
...
const TourInstanceManager = ({ photographerId, tours }) => {
  ...
  return (
    ...
    >
      ...
      >
        📅 투어 일정 오픈{' '}
        <span style={{ color: 'rgba(76,175,80,0.5)' }}>
          — {instances.filter((i) => i.status === 'recruiting').length}개 모집 중
        </span>
      </div>
      <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 16, lineHeight: 1.6 }}>
        투어 날짜를 오픈하면 고객이 참가 신청할 수 있습니다. 마감일까지 인원이 모이면 자동
        확정됩니다.
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 16 }}>
        {instances.length === 0 && (
          <div
            style={{ fontSize: 12, color: 'var(--muted)', textAlign: 'center', padding: '20px 0' }}
          >
            아직 오픈된 투어 일정이 없습니다
          </div>
        )}
          ...
          return (
            ...
            >
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 3 }}>
                    📅 {fmtDate(inst.scheduledDate)} {inst.scheduledTime} &nbsp;|&nbsp; ⏰ 마감:{' '}
                    {fmtDate(inst.deadline)}
                  </div>
                  {/* 투어 간략 정보 */}
                  {(tourData.durationMin || tourData.photos || tourData.desc) && (
                    ...
                    >
                      ...
                      {tourData.route?.departure && (
                        <span>
                          📍 {tourData.route.departure} → {tourData.route.destination || '...'}
                        </span>
                      )}
                    </div>
                  )}
              ...
              <div style={{ marginBottom: 8 }}>
                ...
                >
                  <span>
                    👥 {count}/{inst.maxGuests}명 {perPerson}
                  </span>
                ...
                >
                  <div
                    style={{
                      ...
                      background:
                        count >= inst.maxGuests
                          ? 'var(--success)'
                          : count >= inst.minGuests
                            ? 'var(--gold)'
                            : 'var(--danger)',
                    }}
                  />
      ...
      {instances.filter((i) => i.status === 'recruiting').length > 0 && (
        ...
        >
          <span style={{ color: 'var(--success)', fontFamily: 'var(--font-serif)' }}>
            현재 오픈 날짜:
          </span>{' '}
          {instances
        ...
        >
          ...
          >
            <label style={{ fontSize: 11, color: 'var(--muted)' }}>
              투어 선택
              <select
                value={newForm.tourIndex}
                onChange={(e) => setNewForm({ ...newForm, tourIndex: Number(e.target.value) })}
                style={inputStyle}
              >
                {tours.map((t, i) => (
                  <option key={i} value={i}>
                    {t.name} ({t.pricingType === 'total' ? '총액' : '1인당'} ₩
          ...
          >
            ...
            <label style={{ fontSize: 11, color: 'var(--muted)' }}>
              ...
              <input
                type="number"
                value={newForm.deadlineDays}
                onChange={(e) =>
                  setNewForm({ ...newForm, deadlineDays: Number(e.target.value) || 7 })
                }
                placeholder="7"
                min={1}
                max={30}
              ...
              <span style={{ fontSize: 9, color: 'var(--muted)' }}>
                {newForm.scheduledDate &&
                  `→ 마감일: ${new Date(new Date(newForm.scheduledDate).getTime() - (newForm.deadlineDays || 7) * 86400000).toISOString().slice(5, 10)}`}
              </span>
            </label>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
...
const ArtistSchedule = () => {
  ...
  const toggleExcludeDay = (day) =>
    setExcludeDays((prev) => (prev.includes(day) ? prev.filter((d) => d !== day) : [...prev, day]));
  ...
  useEffect(() => {
    ...
    const hasPortfolio = (profile?.portfolio ?? []).some(
    ...
    if (dbConnected && dbPhotographerId) {
      try {
        ...
        if (sb) {
          ...
          const coverImg =
            portfolioItems[0]?.cover || snaps.find((p) => p.images?.length)?.images?.[0] || null;
  ...
  const activeDayData =
    activeDate && schedule ? getDaySchedule('photographer', artistId, activeDate) : null;
  ...
  const handleApplyRange = async () => {
    ...
    const selectedStr = selectedDays
      .sort((a, b) => a - b)
      .map((d) => dayLabels[d])
      .join('·');
      ...
      <div>
        ...
        >
          ...
          <p style={{ fontSize: 11, color: 'var(--accent-a50)', marginTop: 12, lineHeight: 1.7 }}>
            날짜 범위를 선택하면 해당 기간 전체 시간(00:00~24:00)이 오픈됩니다. 이후 달력에서 날짜
            클릭 → 개별 시간 차단/해제가 가능합니다.
        ...
        >
          ...
          <div>
            ...
            >
              {mainLoc ? (
                <div
                  style={{ fontSize: 12, color: 'var(--text)', fontFamily: 'var(--font-serif)' }}
                >
              ...
              {allLocs.length > 0 &&
                (() => {
                  ...
                  const sortedLocs = [...monthLocs].sort(
                    (a, b) => getLocSortKey(a) - getLocSortKey(b)
                  );
                  return (
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              gap: 5,
                              fontSize: 10,
                              color: loc.active ? 'var(--muted)' : 'rgba(136,136,136,0.4)',
                            }}
                          >
                            <span
                              style={{
                            ...
                            {!loc.isMain &&
                              loc.period?.start &&
                              ` ${loc.period.start}~${loc.period.end || '?'}`}
                            {!loc.active && ' (노출 OFF)'}
                          </div>
                        );
                      })}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2 }}>
                ...
                {Array(daysInMonth)
                  ...
                  .map((_, i) => {
                    ...
                    const locIndices = activeLocs
                      .map((al) => allLocs.findIndex((l) => l.id === al.id))
                      .filter((i) => i >= 0)
                      .slice(0, 3);
                          ...
                          >
                            {locIndices.map((li) => (
                              <span
                                key={li}
                                style={{
                                  width: Math.max(6, Math.floor(28 / locIndices.length)),
                                  height: 3,
                                  background: isActive ? 'var(--text)' : getLocColor(li),
                                  borderRadius: 1,
                                }}
                              />
                            ))}
            ...
            >
              ...
              >
                ...
                <button
                  className={editingDefault ? 'btn-primary' : 'btn-ghost'}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {DEFAULT_TIME_SLOTS.map((time) => {
                  const isOn = (
                    editingDefault
                      ? draftDefault
                      : (schedule?.defaultSlots ?? INITIAL_DEFAULT_HOURS)
                  ).includes(time);
                  return (
                    <button
                      key={time}
                      ...
                      style={{
                        ...
                        cursor: editingDefault ? 'pointer' : 'default',
                        transition: 'all 0.15s',
                      }}
                    >
                      {time}
                    ...
                    <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>
                      {activeDayData.dayOff
                        ...
                        : (() => {
                            ...
                            const effectiveSlots = activeDayData.slots.filter((s) =>
                              dh.includes(s)
                            );
                            const blockedInDefault = activeDayData.blocked.filter((s) =>
                              dh.includes(s)
                            );
                ...
                <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border)' }}>
                  <button
                    ...
                    style={{
                      ...
                      color: activeDayData?.dayOff ? 'var(--danger)' : 'var(--muted)',
                  ...
                  <div style={{ padding: '20px 24px' }}>
                          ...
                          >
                            <span
                              style={{
                                fontFamily: 'var(--font-serif)',
                                fontSize: 15,
                                color: blocked ? 'rgba(136,136,136,0.35)' : 'var(--text)',
                              }}
                            >
                              {time}
                            </span>
                            <span
                              style={{
                                ...
                                letterSpacing: '0.12em',
                                textTransform: 'uppercase',
                                color: blocked ? 'var(--danger)' : 'var(--gold)',
                                padding: '2px 8px',
                                border: `1px solid ${blocked ? 'rgba(232,80,80,0.25)' : 'var(--gold-border)'}`,
                              }}
                            >
                              {blocked ? '차단' : '운영'}
                            </span>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        fontSize: 13,
                        letterSpacing: '0.08em',
                        cursor: slotsDirty ? 'pointer' : 'not-allowed',
                        transition: 'all 0.2s',
                      }}
                    >
  ...
  const renderLocationsTab = () => {
    ...
    const addSubLocation = (newLoc) => {
      if (mainLoc) {
        ...
        if (isCrossType && !mainAlreadyOff) {
          ...
          const otherSameType = locs.filter(
            (l) => !l.isMain && l.locType === mainLoc.locType && l.active !== false
          );
          setMainPauseNotice({
            step: 'pauseMain',
            pendingLoc: newLoc,
    ...
    const LocationCard = ({ loc }) => {
      ...
      const usedByOthers = locs
        .filter((l) => l.id !== loc.id && l.locType === loc.locType)
        .map((l) => l.regionId);
      ...
      const hasUnsaved =
        !isMain && pending && (pending.start !== savedStart || pending.end !== savedEnd);
      ...
      return (
        ...
        >
          ...
          <div style={{ marginBottom: 14, position: 'relative' }}>
            ...
            <button
              onClick={() => {
                if (locPickerOpen === loc.id) {
                  setLocPickerOpen(null);
                  setLocPickerCountry(null);
                  setLocPickerSearch('');
                } else {
                  setLocPickerOpen(loc.id);
                  setLocPickerCountry(isDom ? 'KR' : null);
                  setLocPickerSearch('');
                }
              }}
              style={{
              ...
              >
                ...
                {isDom &&
                  (() => {
                    ...
                    const filtered = DOMESTIC_REGIONS.filter(
                      (r) => !q || r.ko.includes(q) || r.en.toLowerCase().includes(q)
                    );
                    // 즐겨찾기를 위로
                        ...
                        >
                          <input
                            ...
                            onCompositionEnd={(e) => {
                              ...
                              setLocPickerSearch(e.target.value);
                            }}
                            onChange={(e) => {
                              if (!composingRef.current) setLocPickerSearch(e.target.value);
                            }}
                          ...
                          return (
                            <div
                              ...
                              style={{
                                ...
                                opacity: taken ? 0.4 : 1,
                                background: selected ? 'var(--accent-a08)' : 'transparent',
                                borderBottom: '1px solid var(--ink-a03)',
                              }}
                              onClick={() => {
                                if (taken) return;
                                const next = locs.map((l) =>
                                  l.id === loc.id
                                    ? { ...l, regionId: r.id, name: r.ko, nameEn: r.en }
                                    : l
                                );
                                saveProfileData({ ...profile, locations: next });
                                setLocPickerOpen(null);
                        ...
                        {sorted.map(([code, c]) => {
                          ...
                          return (
                            <div
                              ...
                              onClick={() => {
                                setLocPickerCountry(code);
                                setLocPickerSearch('');
                                if (locSearchRef.current) locSearchRef.current.value = '';
                              }}
                            >
                              ...
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  const next = isFav
                                    ? favCountries.filter((id) => id !== code)
                                    : [...favCountries, code];
          ...
          {!isMain && (
            <div style={{ marginBottom: 14 }}>
              ...
              {!hasUnsaved && savedStart && savedEnd && (
                ...
                >
                  <span style={{ color: 'rgba(100,200,100,0.8)' }}>●</span> 일정 저장됨:{' '}
                  {savedStart} ~ {savedEnd}
                  {loc.period?.savedAt ? ` (저장한 날짜: ${loc.period.savedAt})` : ''}
                </div>
              )}
            </div>
          )}
              ...
              >
                ...
                <span
                  style={{
                    ...
                    fontFamily: 'var(--font-serif)',
                    letterSpacing: '0.05em',
                    color: isActive ? 'rgba(80,200,80,0.9)' : 'var(--danger)',
                  }}
                >
    ...
    return (
      <div>
        ...
        >
          ...
          <div>
            <strong style={{ color: 'var(--gold)' }}>메인 활동지</strong>를 반드시 1곳 지정해주세요
            (항상 상주하며 촬영하는 곳).
            <br />
            출장 지역은 날짜 범위를 설정하여 해당 기간에만 고객에게 노출됩니다.
            <br />
            <strong style={{ color: 'var(--text)' }}>국내 최대 3곳, 해외 최대 3곳</strong>까지 추가
            가능합니다.
          </div>
        </div>
          ...
          >
            ...
            {overseas.length < MAX && (
              <button
                className="btn-outline"
                style={{ fontSize: 11, padding: '6px 14px' }}
                onClick={() => addOverseas(artistType)}
              >
                + 해외 지역 추가
              </button>
            )}
        ...
        {mainPauseNotice?.step === 'pauseMain' && (
          ...
          >
            ...
            >
              ...
              >
                출장 기간 동안 메인 활동지({mainPauseNotice.mainLocName})에서의 활동을
                중단하시겠습니까?
              </h2>
              <p style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.8, marginBottom: 24 }}>
                "중단"을 선택하면 해당 기간 메인 활동지에서 고객에게 노출되지 않습니다.
                <br />
              ...
              >
                ⚠ 하루에 두 지역 촬영은 노쇼·사고 위험이 있습니다. 같은 날 두 지역에 예약이 잡힐
                경우, 두 번째 예약은 작가님의 수동 승인이 필요합니다.
              </p>
              <div style={{ display: 'flex', gap: 12 }}>
                <button
                  className="btn-primary"
                  style={{ flex: 1 }}
                  onClick={() => confirmMainPause(true)}
                >
                  중단 (안전)
                </button>
                <button
                  className="btn-outline"
                  style={{ flex: 1 }}
                  onClick={() => confirmMainPause(false)}
                >
                  병행 (두 지역 노출)
                </button>
                <button
                  ...
                  onClick={() => setMainPauseNotice(null)}
                >
                  취소
                </button>
              </div>
        ...
        {mainPauseNotice?.step === 'pauseOthers' && (
          ...
          >
            ...
            >
              ...
              >
                출장 기간 동안 {mainPauseNotice.mainLocType === 'domestic' ? '국내' : '해외'} 타
                지역의 고객 노출도 해제하시겠습니까?
              </h2>
              <p style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.8, marginBottom: 20 }}>
                메인 활동지 외에 {mainPauseNotice.mainLocType === 'domestic' ? '국내' : '해외'}에서
                활동 중인 다른 지역도 고객 노출을 해제할 수 있습니다.
                <br />
                체크된 지역은{' '}
                <strong style={{ color: 'var(--accent-a50)' }}>"고객에게 노출"이 자동 해제</strong>
                되어 검색에 표시되지 않습니다.
              </p>
                  ...
                  return (
                    <label
                      ...
                      style={{
                        ...
                        gap: 12,
                        padding: '12px 16px',
                        border: `1px solid ${checked ? 'var(--accent-a30)' : 'var(--border)'}`,
                        background: checked ? 'var(--accent-a04)' : 'var(--bg)',
                        cursor: 'pointer',
                        transition: 'all 0.15s',
                      }}
                    >
                      <input
                        type="checkbox"
                        checked={checked}
                        onChange={() => togglePauseLoc(loc.id)}
              ...
              >
                * 노출이 해제된 지역은 고객 검색에 표시되지 않습니다.
                <br />* 나중에 해당 지역의 "고객에게 노출"을 다시 체크하면 언제든 복원할 수
                있습니다.
              </p>
        ...
        {crossOverlapNotice && (
          ...
          >
            ...
            >
              ...
              >
                기간 겹침 감지 — {crossOverlapNotice.oppositeType === 'domestic' ? '국내' : '해외'}{' '}
                지역 노출 확인
              </div>
              <h2
                style={{
              ...
              >
                {crossOverlapNotice.triggerType === 'overseas' ? '해외' : '국내'} 출장(
                {crossOverlapNotice.triggerLoc.name}) 기간이
                <br />
                {crossOverlapNotice.oppositeType === 'domestic' ? '국내' : '해외'} 타 지역 활동
                기간과 겹칩니다.
              </h2>
              <p style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.8, marginBottom: 8 }}>
                <span style={{ color: 'var(--gold)' }}>{crossOverlapNotice.triggerLoc.name}</span>{' '}
                활동 기간: {crossOverlapNotice.triggerLoc.period.start} ~{' '}
                {crossOverlapNotice.triggerLoc.period.end ||
                  crossOverlapNotice.triggerLoc.period.start}
              </p>
              <p style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.8, marginBottom: 20 }}>
                겹치는 기간 동안 아래{' '}
                {crossOverlapNotice.oppositeType === 'domestic' ? '국내' : '해외'} 지역의 고객
                노출을 해제하시겠습니까?
                <br />
                체크된 지역은{' '}
                <strong style={{ color: 'var(--accent-a50)' }}>"고객에게 노출"이 자동 해제</strong>
                되어 해당 기간 검색에 표시되지 않습니다.
              </p>
                    ...
                    >
                      <input
                        ...
                        onChange={() => {
                          ...
                          const next = sel.includes(loc.id)
                            ? sel.filter((id) => id !== loc.id)
                            : [...sel, loc.id];
              ...
              {/* 4개 이상 노출 시 경고 */}
              {(() => {
                const activeExposedCount = locs.filter(
                  (l) => l.active !== false && l.period?.start && l.period?.end
                ).length;
                const MAX_EXPOSED = 3;
                ...
                return (
                  <>
                    {overLimit && selectedIds.length === 0 && (
                      ...
                      >
                        ⚠ 고객에게 노출되는 지역이 {activeExposedCount}개입니다. 최대 {MAX_EXPOSED}
                        개까지 동시 노출이 가능합니다.
                        <br />
                    ...
                    <div style={{ display: 'flex', gap: 12 }}>
                      <button
                        ...
                        onClick={() => {
                          if (selectedIds.length > 0) {
                            const updatedLocs = locs.map((l) =>
                              selectedIds.includes(l.id) ? { ...l, active: false } : l
                            );
                      ...
                      <button
                        ...
                        style={{
                          flex: 1,
                          opacity: overLimit && wouldRemain > MAX_EXPOSED ? 0.4 : 1,
                        }}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <button
                  ...
                  onClick={() => {
                    saveProfileData({
                      ...profile,
                      locations: [...locs, { ...hmkLocNotice.pendingLoc, hasHmkPartner: true }],
                    });
        ...
        {hmkLocNotice?.step === 'warn' && (
          ...
          >
            ...
            >
              ...
              <div
                style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.85, marginBottom: 24 }}
              >
                <p style={{ marginBottom: 12 }}>
                  이 지역에서 H&M 아티스트가 동행하지 않는 경우,
                  <br />
                  고객이{' '}
                  <strong style={{ color: 'var(--text)' }}>현지 H&M 아티스트를 직접 섭외</strong>할
                  수 있도록
                  <br />
                  <strong style={{ color: 'var(--gold)' }}>
                    헤어메이크업 비용이 제외된 촬영 전용 상품
                  </strong>
                  으로 등록 바랍니다.
                </p>
                <p>
                  상품 등록 시 "H&M 미포함" 옵션을 선택하면 고객 화면에도 명확히 표시되어
                  <br />
              ...
              <button
                ...
                onClick={() => {
                  saveProfileData({
                    ...profile,
                    locations: [...locs, { ...hmkLocNotice.pendingLoc, hasHmkPartner: false }],
                  });
                  setHmkLocNotice(null);
                }}
              >
  ...
  const renderBookingsTab = () => {
    ...
    return (
      <div>
        ...
        >
          고객이 결제 완료한 예약 요청 목록입니다.
          <br />
          <strong style={{ color: 'var(--gold)' }}>
            48시간 이내에 수락 또는 거절해주세요.
          </strong>{' '}
          미응답 시 예약은 <strong style={{ color: 'var(--danger)' }}>자동 취소</strong>되고
          고객에게 환불됩니다. (촬영일이 임박한 경우 더 일찍 마감될 수 있습니다)
        </div>
          ...
          {pendingBookings.map((b) => (
            ...
            >
              ...
              >
                ...
                <span
                  style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-serif)' }}
                >
                  {b.date} · {b.time}
                </span>
              </div>
  ...
  const renderInfoTab = () => {
    ...
    const addPortfolio = () => {
      const updated = {
        ...
        portfolio: [
          ...portfolio,
          { id: genId(), images: [], coverIdx: 0, caption: '', regionId: '' },
        ],
      };
      saveProfile('photographer', artistId, updated);
      setProfileState(updated);
    ...
    const removePfImage = (pfIdx, imgIdx) => {
      ...
      const next = portfolio.map((p, i) =>
        i === pfIdx ? { ...p, images, coverIdx: newCover } : p
      );
      const updated = { ...profile, portfolio: next };
      saveProfile('photographer', artistId, updated);
      setProfileState(updated);
    ...
    const ALL_REGIONS = [
      ...DOMESTIC_REGIONS.map((r) => ({ ...r, locType: 'domestic' })),
      ...OVERSEAS_REGIONS.map((r) => ({ ...r, locType: 'overseas' })),
    ];
    ...
    const filteredPortfolio =
      pfLocFilter === 'all' ? portfolio : portfolio.filter((pf) => pf.regionId === pfLocFilter);
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
        ...
        >
          ...
          >
            ...
            <div
              style={{
                fontSize: 10,
                color: featuredCount >= 5 ? 'var(--danger)' : 'var(--gold)',
                fontFamily: 'var(--font-serif)',
              }}
            >
              {featuredCount}/5개
          ...
          <p style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 16, lineHeight: 1.7 }}>
            고객이 작가찾기 카드에서 보게 될 대표 이미지입니다. 아래 포트폴리오에서 ★ 대표설정을
            클릭하여 최대 5개의 게시글을 지정하세요.
          </p>
          <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 8 }}>
            {getFeaturedImages().map((item, idx) => (
              <div
                key={idx}
                style={{ position: 'relative', minWidth: 140, width: 140, flexShrink: 0 }}
              >
                <div
                  style={{
                    aspectRatio: '4/3',
        ...
        >
          ...
          >
            ...
            >
              ...
              {!portfolio.some((pf) => (pf.images?.length > 0 || pf.url) && pf.regionId) && (
                <span
                  style={{
                    width: 8,
                    height: 8,
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      background: pf.featured ? 'var(--accent-a03)' : 'var(--bg)',
                      position: 'relative',
                      overflow: 'hidden',
                    }}
                  >
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <button
                            ...
                            style={{
                              ...
                              cursor:
                                !pf.featured && featuredCount >= 5 ? 'not-allowed' : 'pointer',
                          ...
                          {!pf.featured && featuredCount >= 5 && (
                            <span style={{ fontSize: 10, color: 'var(--danger)' }}>최대 5개</span>
                          )}
                        </div>
                        ...
                        <div style={{ position: 'relative' }}>
                          <div
                            style={{
                              fontSize: 10,
                              color: !pf.regionId ? 'var(--danger)' : 'var(--muted)',
                              marginBottom: 4,
                              fontFamily: 'var(--font-serif)',
                              letterSpacing: '0.1em',
                            }}
                          ...
                          <button
                            onClick={() => {
                              if (pfPickerOpen === idx) {
                                setPfPickerOpen(null);
                                setPfPickerCountry(null);
                                setPfPickerSearch('');
                              } else {
                                setPfPickerOpen(idx);
                                setPfPickerCountry(null);
                            ...
                            style={{
                              width: '100%',
                              background: 'var(--bg2)',
                              border: `1px solid ${!pf.regionId ? 'rgba(232,80,80,0.4)' : 'var(--border)'}`,
                              color: 'var(--text)',
                              padding: '7px 10px',
                              fontFamily: 'var(--font-serif)',
                              fontSize: 12,
                            ...
                            >
                              ...
                              {!pfPickerCountry &&
                                (() => {
                                  ...
                                  const allCountries = [
                                    ['KR', COUNTRIES.KR],
                                    ...Object.entries(COUNTRIES).filter(([code]) => code !== 'KR'),
                                  ];
                                    ...
                                    <>
                                      ...
                                      >
                                        <input
                                          ...
                                          defaultValue=""
                                          onCompositionStart={() => {
                                            pfComposingRef.current = true;
                                          }}
                                          onCompositionEnd={(e) => {
                                            pfComposingRef.current = false;
                                            setPfPickerSearch(e.target.value);
                                          }}
                                          onChange={(e) => {
                                            if (!pfComposingRef.current)
                                              setPfPickerSearch(e.target.value);
                                          }}
                                      ...
                                      {sorted.map(([code, c]) => {
                                        const cityCount =
                                          code === 'KR'
                                            ? DOMESTIC_REGIONS.length
                                            : (OVERSEAS_BY_COUNTRY[code] || []).length;
                                        return (
                                          <div
                                            key={code}
                                            style={{
                                            ...
                                            onClick={() => {
                                              setPfPickerCountry(code);
                                              setPfPickerSearch('');
                                              if (pfSearchRef.current)
                                                pfSearchRef.current.value = '';
                                            }}
                                          >
                        ...
                        <textarea
                          value={pf.caption || ''}
                          onChange={(e) => updatePf(idx, 'caption', e.target.value)}
                        ...
                        >
                          <span style={{ fontSize: 11, color: 'var(--muted)' }}>
                            📷 {images.length}/10장{' '}
                            {coverIdx < images.length ? `· ★ ${coverIdx + 1}번 대표사진` : ''}
                          </span>
                          <button
                    ...
                    {isExpanded && (
                      ...
                      >
                        <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 10 }}>
                          사진 관리 (최대 10장) — 끌어서 순서 변경, 맨 앞이 대표사진 (★ 로도 지정
                          가능)
                        </div>
                        {/* 드래그앤드롭 업로드 영역 */}
                        <PortfolioDropZone
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 16 }}>
            <button
              className="btn-primary"
              style={{ fontSize: 12, padding: '10px 28px', letterSpacing: '0.05em' }}
              onClick={async () => {
          ...
          <p
            style={{
              fontSize: 11,
              color: dbConnected ? 'var(--success)' : 'var(--muted)',
              marginTop: 16,
              lineHeight: 1.7,
            }}
          >
            {dbConnected
              ? '✓ Supabase Storage 연동됨 — 이미지가 클라우드에 저장됩니다.'
              : '현재는 미리보기 전용입니다. Supabase 연결 후 클라우드 저장이 활성화됩니다.'}
          </p>
        </div>
        ...
        >
          ...
          <p style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 16, lineHeight: 1.7 }}>
            고객이 작가를 검색할 때 필터에 사용되는 태그입니다. 해당하는 태그를 선택하거나, 직접
            추가할 수 있습니다.
          </p>
                ...
                {allActive.length > 0 && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 16 }}>
                    {allActive.map((filterId) => {
                      const label =
                        SNAP_FILTER_LABELS[filterId]?.ko ||
                        customFilters.find((f) => f.id === filterId)?.label ||
                        filterId;
                          ...
                          <button
                            onClick={() => {
                              if (isCustom) {
                                saveProfileData({
                                  ...profile,
                                  customSnapFilters: customFilters.filter((f) => f.id !== filterId),
                                });
                              } else {
                                saveProfileData({
                                  ...profile,
                                  snapFilters: myFilters.filter((f) => f !== filterId),
                                });
                              }
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        ...
                        saveProfileData({
                          ...profile,
                          customSnapFilters: [...existing, { id, label: val }],
                        });
                        e.target.value = '';
            ...
            {sortedImages.map((_, i) => (
              <div
                ...
                style={{
                  ...
                  height: 6,
                  borderRadius: '50%',
                  background: i === slideIdx ? 'var(--gold)' : 'var(--ink-a30)',
        ...
        >
          ...
          {onToggleFeatured && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                if (!featured && featuredFull) return;
                onToggleFeatured();
              }}
              style={{
                display: 'flex',
      ...
      <div>
        ...
        {onReorder && images.length > 1 && (
          <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 8 }}>
            끌어서 순서를 바꾸세요. <span style={{ color: 'var(--gold)' }}>맨 앞이 대표사진</span>
            입니다.
          </div>
        )}
          ...
          {images.map((img, i) => (
            <div
              ...
              style={{
                ...
                outline:
                  dragTo === i && dragFrom !== null && dragFrom !== i
                    ? '2px solid var(--gold)'
                    : 'none',
                outlineOffset: 2,
              }}
        ...
        {canAdd && (
          ...
          >
            ...
            <div
              style={{
                fontSize: 12,
                color: dragOver ? 'var(--gold)' : uploading ? 'var(--gold)' : 'var(--muted)',
              }}
            >
              {uploading
                ? '⟳ 업로드 중...'
                : dragOver
                  ? '여기에 놓으세요'
                  : `클릭 또는 드래그하여 사진 추가 (${images.length}/${maxCount})`}
            </div>
            <div style={{ fontSize: 10, color: 'var(--muted)', marginTop: 4, opacity: 0.6 }}>
              JPG, PNG, WebP · 최대 5MB
            </div>
          </div>
        )}
        ...
        {images && images.length > 0 && (
          <>
            {onReorder && images.length > 1 && (
              <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 6 }}>
                끌어서 순서를 바꾸세요.{' '}
                <span style={{ color: 'var(--gold)' }}>맨 앞이 대표사진</span>입니다.
              </div>
            )}
              ...
              {images.map((img, imgIdx) => (
                <div
                  ...
                  style={{
                    ...
                    outline:
                      dragTo === imgIdx && dragFrom !== null && dragFrom !== imgIdx
                        ? '2px solid var(--gold)'
                        : 'none',
                    outlineOffset: 2,
                  }}
  ...
  const renderProductsTab = () => {
    if (!profile)
      return (
        <div
          style={{ textAlign: 'center', padding: '60px 0', color: 'var(--muted)', fontSize: 13 }}
        >
          프로필 데이터를 불러오는 중...
        </div>
      );
    ...
    const updateHmk = (field, val) =>
      saveProfileData({ ...profile, hmk: { ...hmk, [field]: val } });
    const addHmkMenu = () =>
      updateHmk('menus', [...hmkMenus, { id: genId(), name: '', price: '', desc: '' }]);
    const updateHmkMenu = (idx, field, val) => {
      const next = hmkMenus.map((m, i) => (i === idx ? { ...m, [field]: val } : m));
      updateHmk('menus', next);
    };
    const removeHmkMenu = (idx) =>
      updateHmk(
        'menus',
        hmkMenus.filter((_, i) => i !== idx)
      );
    ...
    const removePropImage = (idx, imgIdx) => {
      setPropImages({
        ...propImages,
        [idx]: (propImages[idx] || []).filter((_, i) => i !== imgIdx),
      });
    };
    ...
    const addCostume = () =>
      setCostumes([
        ...costumes,
        { id: genId(), name: '', gender: '공용', category: '한복', price: '', desc: '' },
      ]);
    ...
    const handleCostumeImageUpload = (idx, urls) => {
      setCostumeImages({
        ...costumeImages,
        [idx]: [...(costumeImages[idx] || []), ...urls].slice(0, 5),
      });
    };
    const removeCostumeImage = (idx, imgIdx) => {
      setCostumeImages({
        ...costumeImages,
        [idx]: (costumeImages[idx] || []).filter((_, i) => i !== imgIdx),
      });
    };
    ...
    const removeSnapImage = (idx, imgIdx) => {
      setSnapImages({
        ...snapImages,
        [idx]: (snapImages[idx] || []).filter((_, i) => i !== imgIdx),
      });
    };
    ...
    const toggleSnapRegion = (idx, locId) => {
      ...
      const next = regionIds.includes(locId)
        ? regionIds.filter((id) => id !== locId)
        : [...regionIds, locId];
    ...
    const handleSaveSnapProducts = async () => {
      ...
      const mergedSnap = snapProducts.map((p, i) => ({
        ...p,
        images: snapImages[i] || p.images || [],
      }));
      // saveProfileData 는 DB 쓰기 성공 여부를 돌려준다.
      // 기다리지 않고 성공 메시지를 띄우면 실패해도 '저장됨' 이 뜬다.
      const ok = await saveProfileData({
        ...profile,
        snapProducts: mergedSnap,
        props: profile?.props || [],
        costumes: profile?.costumes || [],
        tours: profile?.tours || [],
      });
    ...
    const handleSaveCostumes = async () => {
      ...
      const mergedCostumes = costumes.map((c, i) => ({
        ...c,
        images: costumeImages[i] || c.images || [],
      }));
      // saveProfileData 는 DB 쓰기 성공 여부를 돌려준다.
      // 기다리지 않고 성공 메시지를 띄우면 실패해도 '저장됨' 이 뜬다.
    ...
    const removeTourSpot = (tourIdx, spotIdx) => {
      ...
      updateTour(
        tourIdx,
        'spots',
        (tour.spots || []).filter((_, i) => i !== spotIdx)
      );
    };
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
        ...
        >
          ...
          >
            <input
              type="checkbox"
              checked={hmk.selfAvailable}
              onChange={(e) => updateHmk('selfAvailable', e.target.checked)}
              style={{ accentColor: 'var(--gold)', width: 16, height: 16 }}
            />
            <div>
              <div style={{ fontSize: 13, fontFamily: 'var(--font-serif)' }}>
                자체 H&M 연계 가능
              </div>
              <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>
                체크 시 고객 프로필에 "H&M 포함 가능" 배지가 표시됩니다
              </div>
            </div>
          </label>
          {hmk.selfAvailable && (
            <>
              <textarea
                value={hmk.note}
                onChange={(e) => updateHmk('note', e.target.value)}
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 20 }}>
                ...
                {hmkMenus.length === 0 && (
                  ...
                  >
                    아직 등록된 H&M 메뉴가 없습니다. 메뉴를 추가하면 고객이 예약 시 선택할 수
                    있습니다.
                    <br />
                    <span style={{ fontSize: 11, color: 'var(--muted)', opacity: 0.8 }}>
                      💡 H&M 메뉴를 개별 상품으로 올리지 않아도 됩니다. 스냅 촬영 상품 안에 H&M을
                      포함하여 판매하셔도 괜찮습니다. (선택사항)
                    </span>
                  </div>
                )}
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {hmkMenus.map((menu, idx) => (
                    ...
                    >
                      ...
                      >
                        <input
                          value={menu.name}
                          onChange={(e) => updateHmkMenu(idx, 'name', e.target.value)}
                        ...
                        <input
                          type="number"
                          value={menu.price}
                          onChange={(e) => updateHmkMenu(idx, 'price', e.target.value)}
                          placeholder="가격 (원)"
                          style={inputStyle}
                        />
                      </div>
                      <input
                        value={menu.desc}
                        onChange={(e) => updateHmkMenu(idx, 'desc', e.target.value)}
        ...
        >
          ...
          >
            ...
            >
              📷 스냅 촬영 상품{' '}
              <span style={{ color: 'var(--accent-a50)' }}>— {snapProducts.length}개</span>
                        ...
                        {snapProducts.map((p, i) => (
                          <button
                            ...
                            style={{
                              ...
                              border: `1px solid ${i === snapPreview ? 'var(--gold)' : 'var(--border)'}`,
                              cursor: 'pointer',
                            }}
                          >
                            {p.name || `상품 ${i + 1}`}
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 0 }}>
                      ...
                      >
                        <input
                          value={product.name}
                          onChange={(e) => updateSnapProduct(idx, 'name', e.target.value)}
                        ...
                        >
                          <select
                            ...
                            onChange={(e) => updateSnapProduct(idx, 'duration', e.target.value)}
                            style={{
                              background: 'var(--bg2)',
                              border: '1px solid var(--border)',
                              color: 'var(--text)',
                          ...
                          <input
                            type="number"
                            value={product.editedCount}
                            onChange={(e) => updateSnapProduct(idx, 'editedCount', e.target.value)}
                            placeholder="보정본 수"
                            style={{
                              background: 'var(--bg2)',
                              border: '1px solid var(--border)',
                          ...
                          <input
                            type="number"
                            value={product.price}
                            onChange={(e) => updateSnapProduct(idx, 'price', e.target.value)}
                            placeholder="가격 (원)"
                            style={{
                              background: 'var(--bg2)',
                              border: '1px solid var(--border)',
                        ...
                        <textarea
                          value={product.desc}
                          onChange={(e) => updateSnapProduct(idx, 'desc', e.target.value)}
                          placeholder="상품 설명 (사진 셀렉 안내, 보정 작업 안내, 수정 요청 관련, 파일 전달 방법 관련, 파일 보관 기간 안내 등)"
                          rows={4}
                          style={{
                            width: '100%',
                            background: 'var(--bg2)',
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                          {allLocations.map((loc) => {
                            ...
                            return (
                              <button
                                key={loc.id}
                                onClick={() => toggleSnapRegion(idx, loc.id)}
                                style={{
                                  padding: '3px 8px',
                                  fontSize: 10,
                                  fontFamily: 'var(--font-serif)',
                        ...
                        <PortfolioDropZone
                          ...
                          onUpload={(files) => {
                            ...
                            const process = async () => {
                              ...
                              const newUrls = await uploadImagesToStorage(
                                files,
                                'snap-products',
                                maxNew
                              );
            ...
            {tours.map((tour, idx) => (
              ...
              >
                  ...
                  {[
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background:
                          (tour.pricingType || 'perPerson') === mode.id
                            ? 'var(--accent-a06)'
                            : 'transparent',
                        color: 'var(--text)',
                        fontSize: 11,
                ...
                >
                  <label style={{ fontSize: 11, color: 'var(--muted)' }}>
                    ...
                    <input
                      ...
                      onChange={(e) => updateTour(idx, 'durationMin', Number(e.target.value))}
                      placeholder="90"
                      style={inputStyle}
                    />
                  </label>
                  <label style={{ fontSize: 11, color: 'var(--muted)' }}>
                    ...
                    <input
                      type="number"
                      value={tour.photos}
                      onChange={(e) => updateTour(idx, 'photos', e.target.value)}
                      placeholder="50"
                      style={inputStyle}
                    />
                  </label>
                  <label style={{ fontSize: 11, color: 'var(--muted)' }}>
                    ...
                    <input
                      type="number"
                      value={tour.price}
                      onChange={(e) => updateTour(idx, 'price', e.target.value)}
                      placeholder="300000"
                      style={inputStyle}
                    />
                  </label>
                  <label style={{ fontSize: 11, color: 'var(--muted)' }}>
                    ...
                    <input
                      type="number"
                      value={tour.maxGuests || ''}
                      onChange={(e) => updateTour(idx, 'maxGuests', Number(e.target.value) || 6)}
                      placeholder="6"
                      style={inputStyle}
                    />
                  </label>
                ...
                {(tour.pricingType || 'perPerson') === 'total' && tour.price && tour.maxGuests && (
                  ...
                  >
                    💡 {tour.maxGuests}명 기준 1인당 ₩
                    {Math.round(Number(tour.price) / tour.maxGuests).toLocaleString('ko-KR')}
                  </div>
                )}
                ...
                >
                  ...
                  <textarea
                    value={tour.desc}
                    onChange={(e) => updateTour(idx, 'desc', e.target.value)}
                ...
                <div style={{ marginBottom: 10 }}>
                  ...
                  >
                    <div>
                      ...
                      <input
                        value={tour.route?.departure || ''}
                        onChange={(e) =>
                          updateTour(idx, 'route', {
                            ...(tour.route || {}),
                            ...
                            {pts.length > 1 &&
                              pts.slice(0, -1).map((_, i) => {
                                ...
                                return (
                                  <polygon
                                    key={`arr-${i}`}
                                    points={`${mx - 3},${h / 2 - 3} ${mx + 3},${h / 2} ${mx - 3},${h / 2 + 3}`}
                                    fill="var(--accent-a50)"
                                  />
                                );
                              })}
        ...
        {tours.length > 0 && (
          <TourInstanceManager photographerId={profile.id || artistId} tours={tours} />
        )}
        ...
        >
          ...
          >
            ...
            >
              <span
                style={{
                  color: profile.hourlyRateEnabled === false ? 'var(--danger)' : 'var(--muted)',
                }}
              >
              ...
              <div
                ...
                style={{
                  ...
                  borderRadius: 11,
                  background:
                    profile.hourlyRateEnabled === false ? 'rgba(136,136,136,0.3)' : 'var(--gold)',
                  position: 'relative',
                  transition: 'background 0.2s',
                  cursor: 'pointer',
                }}
          ...
          <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 14, lineHeight: 1.6 }}>
            패키지 외에 시간 단위로 예약받고 싶다면 시간당 가격을 설정하세요. 토글을 끄면 가격이
            저장된 채로 고객에게 노출되지 않습니다.
          </div>
          <div
            style={{
              display: 'grid',
          ...
          >
            ...
            >
              {profile.hourlyRate ? (
                <span
                  style={{ color: 'var(--gold)', fontFamily: 'var(--font-serif)', fontSize: 13 }}
                >
          ...
          {profile.hourlyRateEnabled === false && profile.hourlyRate && (
            ...
            >
              🔒 시간 단위 예약이 비활성화되었습니다. 가격(₩
              {Number(profile.hourlyRate).toLocaleString('ko-KR')})은 저장된 상태이며, 토글을 켜면
              즉시 활성화됩니다.
            </div>
          )}
          {profile.hourlyRateEnabled !== false && profile.hourlyRate && (
            ...
            >
              💡 고객이 프로필에서 "시간 단위 예약" 옵션을 볼 수 있습니다. 최소 1시간부터 예약
              가능합니다.
            </div>
          )}
        </div>
        ...
        >
              ...
              >
                ...
                >
                  <input
                    ...
                    placeholder="소품명 (예: 베일)"
                    style={inputStyle}
                  />
                  <input
                    value={prop.desc}
                    onChange={(e) => updateProp(idx, 'desc', e.target.value)}
                    placeholder="설명 (예: 화이트 플로팅 베일)"
                    style={inputStyle}
                  />
                </div>
                <ImageDropZone
          ...
          <button className="btn-ghost" style={{ fontSize: 12 }} onClick={addProp}>
            + 소품 추가
          </button>
          <div style={{ marginTop: 16, display: 'flex', justifyContent: 'flex-end' }}>
            <button
        ...
        >
                ...
                >
                      ...
                      {['남성', '여성', '공용'].map((g) => (
                        ...
                        >
                          <input
                            type="radio"
                            name={`gender-${idx}`}
                            checked={costume.gender === g}
                            onChange={() => updateCostume(idx, 'gender', g)}
                  ...
                  <label style={{ fontSize: 11, color: 'var(--muted)' }}>
                    ...
                    <select
                      ...
                      onChange={(e) => updateCostume(idx, 'category', e.target.value)}
                      style={{
                        display: 'block',
                        marginTop: 6,
                        width: '100%',
                ...
                >
                  <label style={{ fontSize: 11, color: 'var(--muted)' }}>
                    ...
                    <input
                      type="number"
                      value={costume.price}
                      onChange={(e) => updateCostume(idx, 'price', e.target.value)}
                      placeholder="0"
                      style={inputStyle}
                    />
                  </label>
                  <label style={{ fontSize: 11, color: 'var(--muted)' }}>
                    ...
                    <input
                      value={costume.desc}
                      onChange={(e) => updateCostume(idx, 'desc', e.target.value)}
                      placeholder="예: 플로우 라인, 끈 조절 가능"
                      style={inputStyle}
                    />
                  </label>
                </div>
          ...
          <button className="btn-ghost" style={{ fontSize: 12 }} onClick={addCostume}>
            + 의상 추가
          </button>
          {/* 의상 저장 버튼 */}
          <div style={{ marginTop: 16, display: 'flex', justifyContent: 'flex-end' }}>
  ...
  const renderPaymentTab = () => {
    const pi = paymentDraft ??
      profile?.paymentInfo ?? { bankName: '', accountNumber: '', accountHolder: '', note: '' };
    ...
    return (
      <div style={{ maxWidth: 600 }}>
        ...
        >
          ...
          {[
            {
              step: '01',
              title: '고객 결제',
              desc: '카드 · 카카오페이 · 네이버페이 · 토스페이 → TossPayments로 수령',
            },
            {
              step: '02',
            ...
            <div
              key={item.step}
              style={{ display: 'flex', gap: 16, marginBottom: 14, alignItems: 'flex-start' }}
            >
              <span
                style={{
                  fontFamily: 'var(--font-serif)',
              ...
              <div>
                <div style={{ fontSize: 13, fontFamily: 'var(--font-serif)', marginBottom: 2 }}>
                  {item.title}
                </div>
          ...
          >
            ⚠ 고객에게 개인 계좌번호를 직접 노출하거나 플랫폼 외 결제를 유도하는 행위는 서비스
            이용약관 위반입니다.
            <br />
        ...
        >
          ...
          >
            정산 계좌 등록{' '}
            <span style={{ color: 'rgba(136,136,136,0.4)' }}>— 고객에게 노출되지 않습니다</span>
            {!(pi.bankName && pi.accountNumber && pi.accountHolder) && (
              <span
                style={{
                  width: 8,
                  height: 8,
          ...
          <p style={{ fontSize: 11, color: 'var(--muted)', marginTop: 16, lineHeight: 1.7 }}>
            정산 계좌는 Phosnap 운영팀 검토 후 확인됩니다. 라이브 모드 전환 시 사업자등록증 또는
            신분증 사본이 필요합니다.
          </p>
      ...
      <div>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
            {[
              {
                label: '매출',
                value: formatMoneyFull(thisMonth.revenue),
                change: revChange,
                color: 'var(--gold)',
              },
              {
              ...
              },
              {
                ...
                value: formatMoneyFull(
                  thisMonth.count > 0 ? Math.round(thisMonth.revenue / thisMonth.count) : 0
                ),
                color: 'var(--text)',
              },
              {
        ...
        >
          ...
          <div
            style={{
              ...
              alignItems: 'flex-end',
              flexWrap: 'wrap',
              marginBottom: perfRangeResult ? 16 : 0,
            }}
          >
            <div>
              ...
              <input
                type="date"
                value={perfDateStart}
                onChange={(e) => setPerfDateStart(e.target.value)}
                style={{
                  background: 'var(--bg)',
                  border: '1px solid var(--border)',
                  color: 'var(--text)',
            ...
            <div>
              ...
              <input
                type="date"
                value={perfDateEnd}
                onChange={(e) => setPerfDateEnd(e.target.value)}
                style={{
                  background: 'var(--bg)',
                  border: '1px solid var(--border)',
                  color: 'var(--text)',
            ...
            {perfRangeResult && (
              <button
                ...
                onClick={() => setPerfRangeResult(null)}
              >
                초기화
              </button>
            )}
          ...
          {perfRangeResult && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10 }}>
              {[
                {
                  label: '총 매출',
                  value: formatMoneyFull(perfRangeResult.revenue),
                  color: 'var(--gold)',
                },
                {
                  label: '정산액',
                  value: formatMoneyFull(perfRangeResult.netRevenue),
                  color: 'var(--success)',
                },
                { label: '총 건수', value: `${perfRangeResult.count}건`, color: 'var(--info)' },
                {
                  label: '평균 단가',
                  value: formatMoneyFull(perfRangeResult.avgAmount),
                  color: 'var(--text)',
                },
                ...
                >
                  ...
                  <div style={{ fontSize: 16, fontFamily: 'var(--font-serif)', color: c.color }}>
                    {c.value}
                  </div>
                </div>
              ))}
              ...
              <tbody>
                {monthly.map((m, idx) => {
                  ...
                  return (
                    <tr
                      key={m.yearMonth}
                      style={{ background: idx === 0 ? 'var(--accent-a03)' : 'transparent' }}
                    >
                      <td
                        style={{
                          ...cellStyle,
                      ...
                      >
                        ...
                        {revDelta !== null && (
                          <span
                            style={{
                              fontSize: 9,
                              color: revDelta >= 0 ? 'var(--success)' : 'var(--danger)',
                              marginLeft: 6,
                            }}
                          >
                            {revDelta > 0 ? '↑' : revDelta < 0 ? '↓' : '→'}
                            {Math.abs(revDelta)}%
                          </span>
                        )}
                      </td>
                      ...
                      <td
                        style={{
                          ...
                          textAlign: 'right',
                          fontFamily: 'var(--font-serif)',
                          color: m.collaboCount > 0 ? '#c084fc' : 'var(--muted)',
                        }}
                      >
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          {thisMonth.count === 0 ? (
            ...
            >
              <Corners />
              <div style={{ fontSize: 12, color: 'var(--text)' }}>예측 데이터 분석 중...</div>
            </div>
          )}
        </div>
            ...
            {monthly.length > 0 &&
              (() => {
                ...
                return (
                  <>
                    <div>
                      <span style={{ color: 'var(--gold)' }}>최고 매출 월:</span> {best.yearMonth} (
                      {formatMoneyFull(best.revenue)}, {best.count}건)
                    </div>
                    <div>
                      <span style={{ color: '#c084fc' }}>콜라보 비율:</span> 전체 {totalCnt}건 중{' '}
                      {collaboTotal}건 ({collaboRatio}%)
                    </div>
                    <div>
                      <span style={{ color: 'var(--info)' }}>누적 평균 단가:</span>{' '}
  ...
  const renderCollaboTab = () => {
    ...
    const mainLocArtists =
      myMainLoc && myMainLocActive
        ? filteredArtists.filter((ph) => {
            const theirActiveCities = getTheirActiveCities(ph);
    ...
    const getOverlappingDates = (theirId) => {
      const myTripLocs = myLocs.filter(
        (l) => !l.isMain && l.period?.start && l.period?.end && l.active !== false
      );
        ...
        >
          ...
          <div
            style={{
              ...
              height: 56,
              borderRadius: '50%',
              background: `url(${ph.img}) center/cover`,
              flexShrink: 0,
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              <span style={{ fontSize: 14, fontFamily: 'var(--font-serif)', color: 'var(--text)' }}>
                {ph.nameKo || ph.name}
              </span>
              <span
                style={{
            ...
            <div style={{ fontSize: 11, color: 'var(--muted)' }}>{ph.languages?.join(' · ')}</div>
          </div>
          {/* 제의 버튼 */}
          {showPropose && (
    ...
    return (
      <div>
          ...
          return (
            ...
            >
              ...
              >
                ...
                <div style={{ fontSize: 9, color: 'var(--accent-a50)', marginTop: 3 }}>
                  콜라보 수락 월 {COLLABO_RULES.maxProposalsPerMonth}회 제한
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                <div
                  style={{
                    fontSize: 26,
                    fontFamily: 'var(--font-serif)',
                    color: dailyRemaining <= 0 ? 'var(--danger)' : 'var(--text)',
                    marginBottom: 4,
                  }}
                >
                  {dailyRemaining}
                ...
                <div style={{ fontSize: 9, color: 'var(--accent-a50)', marginTop: 3 }}>
                  확장형 콜라보 제안 1일 {COLLABO_RULES.maxDailyProposals}회 제한
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                <div
                  style={{
                    fontSize: 26,
                    fontFamily: 'var(--font-serif)',
                    color: pendingIn.length > 0 ? 'var(--success)' : 'var(--text)',
                    marginBottom: 4,
                  }}
                >
                  {pendingIn.length}
              ...
              >
                ...
                >
                  {sent.filter((p) => p.status === 'accepted').length}
                </div>
                <div style={{ fontSize: 10, color: 'var(--muted)' }}>성사된 콜라보</div>
                <div style={{ fontSize: 9, color: 'var(--accent-a50)', marginTop: 3 }}>
                  동종 {sameTypeRemaining}/{COLLABO_RULES.maxSameTypePerMonth}회 남음
                </div>
              </div>
            </div>
          );
        ...
        >
          <strong style={{ color: 'var(--gold)' }}>콜라보 규칙</strong>
          <br />· <strong>보완형</strong>: 사진작가 ↔ 영상작가, 사진작가 ↔ H&M 작가, 영상작가 ↔ H&M
          작가 등 이종 콜라보
          <br />· <strong>확장형 (동종)</strong>: 사진+사진, 영상+영상 등 — 월{' '}
          {COLLABO_RULES.maxSameTypePerMonth}회 제한, 메인/서브 역할 지정 필수
          <br />· 제안 발송: 하루 {COLLABO_RULES.maxDailyProposals}회 / 제안 수락: 월{' '}
        ...
        >
          {[
            ...
            <button
              key={sub.key}
              onClick={() => setCollaboView(sub.key)}
              style={{
                padding: '10px 18px',
                fontSize: 11,
                fontFamily: 'var(--font-serif)',
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
              {[
                ...
                <button
                  key={f.key}
                  onClick={() => setCollaboTypeFilter(f.key)}
                  style={{
                    padding: '7px 16px',
                    fontSize: 11,
                    fontFamily: 'var(--font-serif)',
            ...
            {myMainLoc && (
              <div style={{ marginBottom: 32 }}>
                ...
                {mainLocArtists.length > 0 ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {mainLocArtists.map((ph) => (
                      <ArtistCard key={ph.id} ph={ph} />
                    ))}
                  </div>
                ) : (
                  <div
                                ...
                                style={{
                                  ...
                                  background:
                                    o.type === 'main'
                                      ? 'rgba(34,197,94,0.12)'
                                      : 'rgba(34,197,94,0.08)',
                                  border: `1px solid ${o.type === 'main' ? 'rgba(34,197,94,0.25)' : 'rgba(34,197,94,0.15)'}`,
                                }}
                              >
                                {o.city} {o.start} ~ {o.end} ({o.days}일)
                                {o.type === 'main' ? ' · 상주' : ''}
                              </span>
                            ))}
                          </div>
            ...
            {!hasRegions && (
              ...
              >
                ...
                <div style={{ fontSize: 24, marginBottom: 12 }}>📍</div>
                <p
                  style={{ color: 'var(--muted)', fontSize: 13, lineHeight: 1.8, marginBottom: 16 }}
                >
                  콜라보 작가 찾기를 이용하려면
                  <br />
                  먼저 <strong style={{ color: 'var(--gold)' }}>활동 지역</strong> 탭에서 활동
                  지역을 등록해주세요.
                </p>
                <button
        ...
        {collaboView === 'received' && (
          <div>
            {received.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {received.map((p) => (
                  <ProposalCard key={p.id} proposal={p} direction="received" />
                ))}
              </div>
            )}
          </div>
        ...
        {collaboView === 'sent' && (
          <div>
            {sent.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {sent.map((p) => (
                  <ProposalCard key={p.id} proposal={p} direction="sent" />
                ))}
              </div>
            )}
          </div>
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: 48,
                        borderRadius: '50%',
                        background: `url(${target.img}) center/cover`,
                        flexShrink: 0,
                      }}
                    />
                    <div>
                      <div style={{ fontSize: 14, fontFamily: 'var(--font-serif)' }}>
                        {target.nameKo || target.name}
                      </div>
                      <div style={{ fontSize: 11, color: 'var(--muted)' }}>
                        {targetType?.icon} {targetType?.ko} · {target.location}
                      </div>
                    </div>
                  </div>
                        ...
                        <input
                          ...
                          onChange={() => {
                            ...
                            if (ts && te && sumEl) {
                              ...
                              if (dailyH > 0)
                                sumEl.textContent = `일 ${dailyH.toFixed(1)}시간 × ${days}일 = 총 ${(dailyH * days).toFixed(1)}시간`;
                            }
                          }}
                        />
                        <input
                  ...
                  <div style={{ marginBottom: 20 }}>
                    <label style={{ fontSize: 11, color: 'var(--muted)' }}>
                      ...
                      <textarea
                        id="collabo-message"
                        rows={3}
                        placeholder="예: 교토 웨딩 촬영건으로 H&M 작가 아티스트를 찾고 있습니다. 함께 작업해보고 싶습니다!"
                        style={{
                          display: 'block',
                          marginTop: 6,
                          width: '100%',
                  ...
                  <div style={{ display: 'flex', gap: 12 }}>
                    <button
                      ...
                      onClick={() => {
                        ...
                        // 시간 체크
                        const timeStart =
                          document.getElementById('collabo-time-start')?.value || '10:00';
                        const timeEnd =
                          document.getElementById('collabo-time-end')?.value || '14:00';
                    ...
                    <button
                      ...
                      onClick={() => setCollaboProposalPopup(null)}
                    >
                      취소
                    </button>
                  </div>
        ...
        {collaboRejectPopup && (
          ...
          >
            ...
            >
              ...
              <textarea
                value={collaboRejectReason}
                onChange={(e) => setCollaboRejectReason(e.target.value)}
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                <button
                  ...
                  onClick={() => handleReject(collaboRejectPopup.proposalId)}
                >
                  거절 확정
                </button>
                <button
    ...
    <div className="page-enter" style={{ paddingTop: 100 }}>
      ...
      <div className="section">
          ...
          const cards = [
            {
              ...
              sub: `정산 완료: ${formatMoney(totalNet)}`,
              color: 'var(--gold)',
            },
            {
              label: '총 누적 건수',
              value: `${totalCnt}건`,
              sub: `평균 단가: ${formatMoney(totalCnt > 0 ? Math.round(totalRev / totalCnt) : 0)}원`,
              color: 'var(--info)',
            },
            {
              label: '이번 달 매출',
              ...
              sub:
                revDiff !== 0
                  ...
                  : '전월 데이터 없음',
              color: revDiff >= 0 ? 'var(--success)' : 'var(--danger)',
            },
            {
              label: '이번 달 건수',
              value: `${thisMonth.count}건`,
        ...
        {(() => {
          ...
          // 전체 미완료 수 (상단에 안내 배너)
          const setupMissing = [!hasMainLoc, !hasPortfolio, !hasSnapProduct, !hasPayment].filter(
            Boolean
          ).length;
            ...
            <>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  >
                    ...
                    <span style={{ flex: 1, textAlign: 'left' }}>
                      고객에게 노출되려면 <strong>{setupMissing}개</strong> 필수 항목을
                      입력해주세요. 빨간 점이 표시된 탭을 확인하세요.
                    </span>
                    <span
                      style={{
                        fontSize: 14,
                        transition: 'transform 0.2s',
                        transform: requiredItemsExpanded ? 'rotate(180deg)' : 'rotate(0deg)',
                      }}
                    >
                      ▼
                    </span>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {requiredItems.map((item, idx) => (
                          ...
                          >
                            <span
                              style={{
                                ...
                                background: item.completed
                                  ? 'rgba(76,181,100,0.2)'
                                  : 'rgba(232,80,80,0.2)',
                                color: item.completed ? '#4cb564' : 'var(--danger)',
                                fontWeight: 'bold',
                                fontSize: 11,
                              }}
                            >
                            ...
                            <span
                              style={{ color: item.completed ? 'var(--muted)' : 'var(--danger)' }}
                            >
                              {item.name}
                            </span>
                          </div>
              ...
              >
                {[
                  ...
                  {
                    key: 'collabo',
                    label: `콜라보${collaboReceived.length > 0 ? ` (${collaboReceived.length})` : ''}`,
                  },
                  {
                    key: 'bookings',
                    label: `예약 요청${pendingBookings.length > 0 ? ` (${pendingBookings.length})` : ''}`,
                  },
                ].map((tab) => (
                  <button
                    key={tab.key}
                    ...
                    {tabAlerts[tab.key] && (
                      <span
                        style={{
                          ...
                          borderRadius: '50%',
                          background:
                            tab.key === 'bookings' || tab.key === 'collabo'
                              ? 'var(--accent-deep)'
                              : 'var(--danger)',
                          display: 'inline-block',
                        }}
