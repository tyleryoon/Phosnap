...
const AdminDashboard = () => {
  ...
  const i18n = {
    ko: {
      ...
      monthlyRevenue: '이번 달 거래액',
      monthlyCommission: '이번 달 수수료 수익',
      newSignups: '신규 가입',
    ...
    ja: {
      ...
      members: 'メンバー',
      totalBookings: '合計予約数',
      monthlyRevenue: '月間取扱高',
      monthlyCommission: '月間手数料収入',
      newSignups: '新規登録',
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const [bookingsRes, profilesRes, photogRes, stylistRes, pkgRes, dressRes, dvRes] =
          await Promise.all([
            sb.from('bookings').select('*').order('created_at', { ascending: false }),
            sb.from('profiles').select('*').order('created_at', { ascending: false }),
            sb
              .from('photographers')
              .select('id, user_id, artist_type, hmk_self, dress_self, is_active'),
        ...
        const hmkPkgCount = countBy(
          (pkgRes.data || []).filter((x) => x.type === 'hmk'),
          'photographer_id'
        );
        const costumePkgCount = countBy(
          (pkgRes.data || []).filter((x) => x.type === 'costume'),
          'photographer_id'
        );
        ...
        setStats({
          ...
          // 거래액 — 고객이 낸 총액. 대부분은 작가·헤메·벤더에게 간다.
          monthlyRevenue: bookingsData
            .filter((b) => inThisMonth(b.created_at))
            .reduce((sum, b) => sum + (b.total_price || 0), 0),
          // 수수료 수익 — Phosnap 이 실제로 버는 돈.
          //
          // 예전에는 거래액 하나만 '이번 달 매출' 로 보여줬다. 관리자가
          // 그 숫자를 회사 매출로 읽으면 실제의 여덟 배로 착각한다
          // (103만 vs 13만). 둘은 다른 돈이므로 따로 보여준다.
          monthlyCommission: bookingsData
            .filter((b) => inThisMonth(b.created_at))
        ...
        setBookings([]);
        setProfiles([]);
        setStats({ totalBookings: 0, monthlyRevenue: 0, monthlyCommission: 0, newSignups: 0, activePhotographers: 0 });
      } finally {
        setLoading(false);
      }
    };
                      ...
                      onChange={(e) => {
                        ...
                        setBookings(
                          bookings.map((booking) =>
                            booking.id === b.id ? { ...booking, status: e.target.value } : booking
                          )
                        );
                      }}
                      style={{
  ...
  const CapBadge = ({ label, on, count }) => {
    ...
    return (
      <span
        title={
          empty
            ? '보유로 표시했지만 등록된 항목이 0개입니다 — 고객 화면에서 빈칸으로 보입니다'
            : unknown
              ? '개수를 확인하지 못했습니다'
              : undefined
        }
                    ...
                    {(() => {
                      ...
                      return (
                        <>
                          ...
                          <CapBadge
                            label="자체 헤메"
                            on={cap.hmkSelf && cap.kind === 'photographer'}
                            count={cap.hmkCount}
                          />
      ...
      <div className="section">
        ...
        {!loadError &&
          ...
          !['approvals', 'inquiries'].includes(activeTab) && (
            ...
            >
              조회 결과가 비어 있습니다. 실제로 데이터가 없거나, 관리자 읽기 정책(RLS)이 없어
              걸러졌을 수 있습니다.
            </div>
          )}
