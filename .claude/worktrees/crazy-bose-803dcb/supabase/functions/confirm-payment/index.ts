...
serve(async (req: Request) => {
  ...
  try {
    ...
    // ── 4-1. 금액 분해 ──
    // 신규 정책:
    //   total_price (= amount)  =  subtotal + service_fee
    //   subtotal                =  package_price + stylist_price + dress_price + venue_price
    //   package_price            =  subtotal - (stylist + dress + venue)
    //   service_fee              =  customer-paid 4% (platform revenue, not artist payout)
    const stylistPriceNum = Number(stylistPrice || 0);
    const dressPriceNum   = Number(dressPrice   || 0);
    const venuePriceNum   = Number(venuePrice   || 0);
    const totalPriceNum   = Number(amount);
    const serviceFeeNum   = serviceFee != null ? Number(serviceFee) : 0;
    const subtotalNum     = subtotal != null
      ? Number(subtotal)
      : Math.max(0, totalPriceNum - serviceFeeNum);
    const packagePriceNum = Math.max(
      0,
      subtotalNum - stylistPriceNum - dressPriceNum - venuePriceNum,
    );
    ...
    const { data: booking, error: insertError } = await supabaseAdmin
      ...
      .insert([{
        ...
        package_price:            packagePriceNum,
        stylist_price:            stylistPriceNum,
        dress_price:              dressPriceNum,
        venue_price:              venuePriceNum,
