...
const Booking = () => {
  ...
  const subtotalPrice  = (pkgData?.price || 0) + (stylistSvcData?.price || 0) + dressPrice + venuePrice;
  const { serviceFee, total: totalPrice } = calculateCustomerTotal(subtotalPrice);
  const serviceFeeLabel = SERVICE_FEE_LABEL[lang] || SERVICE_FEE_LABEL.ko;
  const serviceFeeDesc  = SERVICE_FEE_DESCRIPTION[lang] || SERVICE_FEE_DESCRIPTION.ko;
          ...
          <div>
            ...
            {step === 6 && (
              <div>
                ...
                {/* 결제 금액 분해: 소계 + 서비스 수수료 + 합계 */}
                <div style={{ borderTop: '1px solid var(--border)', padding: '20px 0 8px', marginBottom: 24 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10 }}>
                    <span style={{ fontFamily: 'var(--font-serif)', fontSize: 12, letterSpacing: '0.1em', color: 'var(--muted)', textTransform: 'uppercase' }}>
                      {lang === 'ko' ? '소계' : lang === 'ja' ? '小計' : lang === 'zh' ? '小计' : 'Subtotal'}
                    </span>
                    <span style={{ fontFamily: 'var(--font-serif)', fontSize: 14, color: 'var(--text)' }}>₩{fmt(subtotalPrice)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
                    <span style={{ fontFamily: 'var(--font-serif)', fontSize: 12, letterSpacing: '0.06em', color: 'var(--muted)' }}>
                      {serviceFeeLabel} ({Math.round(CUSTOMER_SERVICE_FEE_RATE * 100)}%)
                    </span>
                    <span style={{ fontFamily: 'var(--font-serif)', fontSize: 14, color: 'var(--text)' }}>₩{fmt(serviceFee)}</span>
                  </div>
