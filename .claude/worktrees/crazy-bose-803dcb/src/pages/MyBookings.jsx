...
import PointRewardToast from '../components/PointRewardToast';
...
import { calculateReviewPoints, addPoints } from '../lib/points';
...
const MyBookings = () => {
  ...
  const awardReviewPoints = async (bookingId, reviewData) => {
    if (!user?.id) return;
    const result = calculateReviewPoints(reviewData);
    if (result.total <= 0) return;
    const res = await addPoints(user.id, result.total, 'review_reward', bookingId);
    if (res?.success) setPointToast(result);
  };
  ...
  const handlePkgReviewConfirm = async ({ rating, title, body }) => {
    ...
    if (error) {
      ...
      // Treat package reviews as the artist track for points (single rating).
      await awardReviewPoints(pkgReviewTarget.id, {
        artist: { rating, tags: [], comment: body },
      });
  ...
  const handleArtistReviewConfirm = async ({ rating, title, body, tags }) => {
    ...
    if (error) {
      ...
      await awardReviewPoints(artistReviewTarget.id, {
        artist: { rating, tags: tags || [], comment: body },
      });
  ...
  return (
    <div className="page-enter" style={{ paddingTop: 100 }}>
      ...
      {pointToast && (
        <PointRewardToast
          amount={pointToast.total}
          details={pointToast.detail}
