...
import PointRewardToast from './PointRewardToast';
...
const UnifiedReviewModal = ({ booking, onClose, onSaved }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Calculate and award points (uses authenticated user, not booking metadata)
      try {
        const { calculateReviewPoints, addPoints, POINT_RULES } = await import('../lib/points');
        const pointResult = calculateReviewPoints(reviewData);
        const userId = user?.id || booking?.userId || booking?.user_id;
        if (pointResult.total > 0 && userId) {
          const awarded = await addPoints(userId, pointResult.total, 'review_reward', booking.id);
          if (awarded?.success) {
            const labels = POINT_RULES?.[lang] || POINT_RULES?.ko || {};
            const labelled = (pointResult.detail || []).map((d) => ({
              ...d,
              label: labels[d.key] || d.key,
            }));
            setPointAnimation({ ...pointResult, detail: labelled });
          }
        }
      } catch (err) {
        // silently handled — review save still succeeds
      }
  ...
  return (
    ...
    >
      <div style={{
        ...
        {pointAnimation && (
          <PointRewardToast
            amount={pointAnimation.total}
            details={pointAnimation.detail}
            onDismiss={() => setPointAnimation(null)}
          />
        )}
